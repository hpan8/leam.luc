...
def main():
    ...
    if opts.mode == 'putfolder':
        check_args(args, 1)
        site.createFolder(args[0], opts.title)

    elif opts.mode == 'putfile':
        check_args(args, 1)
        site.putFileURL(args[0], opts.url, opts.title)

    elif opts.mode == 'getfile':
        check_args(args, 1)
        site.getFile(opts.url, args[0])

    elif opts.mode == 'putimage':
        check_args(args, 1)
        site.putImageURL(args[0], opts.url, opts.title)
        ...
        check_args(args, 1)
        site.putDocument(args[0], opts.url, opts.title)
